...
import { sectionLines, summaryText, type SchemeDetailRecord } from '@/lib/schemes/details';
...
const detailById = new Map((details.schemes as unknown as SchemeDetailRecord[]).map((scheme) => [scheme.id, scheme]));
...
export default function SchemeDetail({ browseMode = false, language, location, match, onBack, onDashboard, schemeId }: SchemeDetailProps) {
  ...
  return <main className="min-h-screen bg-[#f5f8fc] text-[#111]">
    <header className="border-b border-[#dce3eb] bg-white"><div className="mx-auto flex min-h-16 max-w-[1180px] items-center justify-between gap-4 px-5 sm:px-8 lg:px-10">
      <button aria-label="Go to dashboard" className="text-xl font-bold tracking-[0.08em] text-[#123f76] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#2458a6]" onClick={onDashboard} type="button">UMANG</button>
    ...
    <div className="mx-auto w-full max-w-[1060px] px-5 py-7 sm:px-8 sm:py-9 lg:px-10">
      ...
      <section className="mt-5 rounded-2xl border border-[#b9cee5] bg-white p-5 shadow-[0_8px_24px_rgba(26,77,132,0.08)] sm:p-7">
        <span className="inline-flex rounded-full bg-[#dcecff] px-2.5 py-1 text-[11px] font-bold tracking-[0.08em] text-[#08478f]">{browseMode ? 'SCHEME DETAILS' : `${match.relevance} MATCH`}</span>
        ...
        {!browseMode ? <div className="mt-4 flex flex-wrap items-center gap-3 border-t border-[#e1e7ee] pt-4 text-sm">
      ...
      <div className="mt-5 grid gap-5 lg:grid-cols-2">
        ...
        {!browseMode ? <Section title={copy.why}><SourceList empty={copy.missing} items={match.matchedCriteria} /></Section> : null}
        {!browseMode ? <Section title={copy.stillToCheck}><SourceList empty={copy.missing} items={unresolved} /></Section> : null}
